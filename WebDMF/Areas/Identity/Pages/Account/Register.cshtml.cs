...
namespace WebDocumentManagement_FileSharing.Areas.Identity.Pages.Account
{
    ...
    {
        // 1. Đổi tất cả IdentityUser thành ApplicationUser
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                user.IsPremium = false; // Mặc định chưa mua
                ...
                if (result.Succeeded)
                {
                    ...
                    await _emailService.SendEmailAsync(Input.Email, "Xác nhận tài khoản",
                        $"Vui lòng xác nhận tài khoản bằng cách <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>Nhấn vào đây</a>.");

                    if (_userManager.Options.SignIn.RequireConfirmedAccount)
                    {
                        return RedirectToPage("/Account/Login", new { area = "Identity", email = Input.Email, requireConfirmation = true, returnUrl = returnUrl });
                    }
                    else
                    {
        ...
        // Đổi Helper trả về IUserEmailStore<ApplicationUser>
